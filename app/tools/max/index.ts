...
function reportData(url: string, data: MaxData): Record<string, unknown> {
  ...
  return {
    ...
    summary: `Max ran Lighthouse SEO on the Daytona preview, scored ${data.seo_score}%, found ${issueCount} issue${issueCount === 1 ? "" : "s"}, and proposed ${data.suggestions.length} fix${data.suggestions.length === 1 ? "" : "es"}.`,
